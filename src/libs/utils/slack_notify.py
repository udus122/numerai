...
def slack_notify(notify_type="1", title=None, value=None, run_name=None, ts=None):
    ...
    if notify_type == "1":
        ...
        value = "{}python submit_kaggle.py --sub_file={}/{}_submission.csv{}".format(
            "`", run_name, run_name, "`"
        )
    elif notify_type == "3":
        message = "<@U7KG6AE8Y>"
    ...
    data = {
        ...
        "attachments": json.dumps(
            [{"color": color, "fields": [{"title": title, "value": value}]}]
        ),
        "thread_ts": ts,
    }
...
def slack_file_upload(file, ts=None):
    ...
    data = {"token": TOKEN, "channels": CHANNEL, "title": filename, "thread_ts": ts}
...
if __name__ == "__main__":
    slack_notify(
        notify_type="2",
        title="hello numerak slack",
        value="yeah from my local pc",
        run_name="sample",
    )
